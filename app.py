...
def search():
    ...
    if filter_display_list_to_dict is not None:
        return jsonify(
            {"code": 6200, "message": "搜索成功", "data": filter_display_list_to_dict,
...
def createDataSet():
    dataSet = [[2, 0, 0, 0, 2, 0, 0, 0, 'no'],
               ...
               [2, 1, 0, 2, 2, 1, 1, 0, 'no'],
               [0, 0, 0, 0, 2, 0, 1, 1, 'no']]
    labels = ['age', 'height', 'edu', 'salary', 'marital', 'occupation', 'house', 'car']  # 特征标签
...
def chooseBestFeatureToSplit(dataSet):
    ...
    for i in range(numFeatures):  # 遍历所有特征
        ...
        print("第%d个特征的增益为%.3f - %.3f =%.3f" % (i + 1, baseEntropy, newEntropy, infoGain))
...
def plotNode(nodeTxt, centerPt, parentPt, nodeType):
    ...
    font = FontProperties(fname=r"c:\windows\fonts\simsunb.ttf", size=14)  # 设置中文字体失败转而用英文
